...
function readFrameRate(value) {
  const candidates = [value.fps, value.FPS, value.frameRate, value.framerate, value.frame_rate];
...
export function collectVariants(value, out=[], seen=new Set()) {
  ...
  const fps=readFrameRate(value);
  const url=value.url||value.UrlList||value.urlList||value.playAddr||value.play_url||value.downloadAddr;
  if ((width||height||bitrate||fps) && (typeof url==='string'||Array.isArray(url))) out.push({width,height,bitrate,fps,codec:value.codecType||value.codec||null});
...
export default async function handler(req,res){
  ...
  try{const response=await fetch(target,{redirect:'follow',headers:{'user-agent':'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 Chrome/124 Safari/537.36','accept-language':'en-US,en;q=0.9'}});if(!response.ok)throw new Error(`TikTok returned ${response.status}`);const finalUrl=new URL(response.url);if(!ALLOWED.test(finalUrl.hostname))throw new Error('Unexpected redirect');const html=await response.text();if(html.length>8_000_000)throw new Error('Page response is too large');const state=extractJson(html,'__UNIVERSAL_DATA_FOR_REHYDRATION__')||extractJson(html,'SIGI_STATE');if(!state)throw new Error('TikTok did not expose readable public playback data');const variants=collectVariants(state).filter((v,i,a)=>i===a.findIndex(x=>x.width===v.width&&x.height===v.height&&x.bitrate===v.bitrate&&x.fps===v.fps)).sort((a,b)=>b.bitrate-a.bitrate).slice(0,12);const frameRates=[...new Set(variants.map(v=>v.fps).filter(Boolean))].sort((a,b)=>b-a);const title=(html.match(/<title>(.*?)<\/title>/i)?.[1]||'TikTok video').replace(/<[^>]+>/g,'').slice(0,180);return res.status(200).json({title,variants,frameRates});}catch(e){return res.status(502).json({error:e.message||'Could not inspect this public video'})}
}
